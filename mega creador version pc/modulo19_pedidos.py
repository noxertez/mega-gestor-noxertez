...
ESTADOS_KANBAN = [
    ('por_empezar', "#cfe2ff", "#084298"),
    ('en_proceso', "#fff3cd", "#856404"),
    ('montado', "#f8d7da", "#721c24"),
    ('tintado', "#e0e7ff", "#3730a3"),
    ('barnizado', "#d1e7dd", "#0f5132"),
    ('listo_para_entregar', "#d2f4ea", "#0a58ca")
]
...
class GestorPedidos:
    ...
    def obtener_pedidos(self):
        ...
        try:
            ...
            # Cambiamos JOIN por LEFT JOIN para que aparezcan pedidos sin cliente vinculado (usando nombre_cliente de la tabla pedidos)
            cursor.execute('''SELECT p.*, 
                            COALESCE(c.nombre, p.nombre_cliente, 'Sin nombre') as cliente_nombre, 
                            pr.FOTO_PORTADA, pr.NOMBRE as articulo_nombre
                            FROM pedidos p 
    ...
    def actualizar_estado(self, id_pedido, nuevo_estado):
        ...
        try:
            ...
            elif nuevo_estado == 'listo_para_entregar':
...
class TabPedidos(tk.Frame):
    ...
    def _crear_interfaz(self):
        ...
        for nombre, bg, fg in ESTADOS_KANBAN:
            ...
            tk.Label(header, text=nombre.replace('_', ' ').upper(), font=("Segoe UI", 11, "bold"), bg=bg, fg=fg).pack()
    ...
    def cargar_datos(self):
        ...
        for p in pedidos:
            ...
            if p['estado'] != 'listo_para_entregar' and p['estado'] != 'entregado':
                contador_hoy += 1
    ...
    def _crear_tarjeta(self, pedido):
        ...
        fecha_val = pedido.get('fecha_pedido')
        fecha_str = ""
        if fecha_val:
    ...
    def ver_detalle_pedido(self, pedido):
        ...
        def guardar_checks():
            ...
            try:
                cursor = conn.cursor()
                cursor.execute("UPDATE pedidos SET unboxing_checklist = %s WHERE id = %s", (json.dumps(final_list), pedido['id']))
                conn.commit()
    ...
    def abrir_crear_pedido(self):
        ...
        def save():
            ...
            try:
                cursor = conn.cursor()
                cursor.execute("INSERT INTO pedidos (id_cliente, fecha_pedido, prioridad, detalles_criticos, estado) VALUES (%s,%s,%s,%s,%s)",
                            (id_cli, now, prio, crit, 'por_empezar'))
                conn.commit()
            finally:
                if conn: conn.close()
            win.destroy()
