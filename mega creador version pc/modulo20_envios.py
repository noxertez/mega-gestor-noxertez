...
class TabEnvios(tk.Frame):
    ...
    def cargar_clientes_combo(self):
        try:
            ...
            print(f"Error cargar_clientes_combo: {e}")
    ...
    def on_manual_cliente_select(self, event):
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE p.id_cliente = %s AND p.estado != 'Entregado'
            ...
            if pedido:
                ...
                cursor.execute("SELECT codigo_postal, direccion FROM clientes WHERE id = %s", (id_cliente,))
            ...
            cursor.execute("SELECT p.NOMBRE, p.SKU_REF FROM productos p JOIN cliente_articulos ca ON p.SKU_REF = ca.sku_articulo WHERE ca.id_cliente = %s", (id_cliente,))
    ...
    def on_manual_articulo_select(self, event):
        ...
        try:
            ...
            if not conn: return
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT peso_envio, largo_envio, ancho_envio, alto_envio FROM productos WHERE SKU_REF = %s", (sku,))
            prod = cursor.fetchone()
            conn.close()
            if prod: self._rellenar_dimensiones(prod)
        except Exception as e:
            print(f"Error on_manual_articulo_select: {e}")
    ...
    def buscar_articulo_catalogo(self):
        ...
        def buscar(e=None):
            ...
            if not conn: return
            cursor = conn.cursor()
            cursor.execute("SELECT SKU_REF, NOMBRE FROM productos WHERE SKU_REF LIKE %s OR NOMBRE LIKE %s LIMIT 50", (q, q))
    ...
    def on_pedido_select(self, event):
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM pedidos p JOIN clientes c ON p.id_cliente = c.id
                LEFT JOIN productos pr ON p.sku_articulo = pr.sku_ref WHERE p.id = %s
            ...
            print(f"Error on_pedido_select: {e}")
        ...
        if res:
            ...
            if self.pedido_seleccionado:
                try:
                    ...
                    if not conn: return
                    cursor = conn.cursor()
                    cursor.execute("UPDATE pedidos SET estado='Entregado', costo_envio=%s, metodo_envio=%s, tracking_id=%s WHERE id=%s", 
                                (t['precio'], t['empresa'], res['tracking'], self.pedido_seleccionado))
                    conn.commit(); conn.close()
                except Exception as e:
                    print(f"Error procesar_envio: {e}")
            self.cargar_pedidos_listos()
    ...
    def cargar_pedidos_listos(self):
        ...
        try:
            ...
            if not conn: return
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT p.*, c.nombre FROM pedidos p JOIN clientes c ON p.id_cliente = c.id WHERE p.estado='Listo para entrega'")
            ...
            print(f"Error cargar_pedidos_listos: {e}")
