...
try:
    from n8n.modulo_n8n import consultar_asistente
except ImportError:
    def consultar_asistente(*args, **kwargs):
        return "Asistente no disponible (módulo n8n no encontrado)"
...
<body>
    ...
    <script>
        ...
        function switchTab(tab) {
            ...
            // Ocultar buscador si estamos en notas
            const searchArea = document.querySelector('.search-area');
            if (searchArea) {
                searchArea.style.display = (tab === 'tareas') ? 'none' : 'flex';
            }
...
def api_clientes():
    ...
    try:
        ...
        sql = "SELECT * FROM clientes WHERE nombre LIKE %s OR telefono LIKE %s OR ciudad LIKE %s"
...
def api_clientes_guardar():
    data = request.json
    conn = get_db_connection()
    if not conn: return jsonify({"success": False, "error": "DB Connection Error"}), 500
    try:
        cursor = conn.cursor()
        if data.get('id'):
            cursor.execute("""
                UPDATE clientes SET nombre=%s, telefono=%s, email=%s, instagram=%s, direccion=%s, ciudad=%s, codigo_postal=%s, notas=%s
                WHERE id=%s
...
def api_pedidos():
    ...
    try:
        ...
        sql = """
            SELECT p.*,
                   COALESCE(c.nombre, '🎁 Influencer / Prueba') as cliente_nombre
            FROM pedidos p
            LEFT JOIN clientes c ON p.id_cliente = c.id
            WHERE (c.nombre LIKE %s OR p.detalles_criticos LIKE %s OR p.notas LIKE %)
            ORDER BY p.id DESC
        """
        # MySQL no soporta el escape unicode \U0001f381 directamente en string SQL de esta forma si el charset no es utf8mb4 con soporte total
        # pero podemos usar el emoji directamente o COALESCE con texto plano si hay dudas. 
        # Mantendremos la lógica pero con placeholder compatible.
        cursor.execute("""
            SELECT p.*,
                   COALESCE(c.nombre, '🎁 Influencer / Prueba') as cliente_nombre
            FROM pedidos p
            LEFT JOIN clientes c ON p.id_cliente = c.id
            WHERE (c.nombre LIKE %s OR p.detalles_criticos LIKE %s OR p.notas LIKE %s)
            ORDER BY p.id DESC
...
def api_pedidos_guardar():
    ...
    try:
        ...
        if data.get('id'):
            cursor.execute("""
                UPDATE pedidos SET id_cliente=%s, prioridad=%s, estado=%s, detalles_criticos=%s
                WHERE id=%s
...
def api_pedidos_mover():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT estado FROM pedidos WHERE id=%s", (data['id'],))
        row = cursor.fetchone()
        if not row: return jsonify({"success": False, "error": "No encontrado"})
        
        estados = ["Por empezar", "En proceso", "Secado/Horno", "Acabado/Barniz", "Listo para entrega", "Entregado"]
        try:
...
def api_futuros_enviar_produccion():
    ...
    try:
        ...
        cursor.execute('SELECT * FROM futuros_proyectos WHERE id=%s', (futuro_id,))
        futuro = cursor.fetchone()
        if not futuro:
            return jsonify({"success": False, "error": "Proyecto no encontrado"})
        # futuro ya es dict gracias a dictionary=True

        # Asegurar columnas necesarias (MySQL compatible)
        ...
        cursor.execute("UPDATE futuros_proyectos SET ESTADO='En Proceso' WHERE id=%s", (futuro_id,))
...
def buscar():
    ...
    try:
        ...
        tel = row_tel['valor'] if row_tel else ""

        cat = request.args.get('cat', '')
        sql = "SELECT * FROM productos WHERE (SKU_REF LIKE %s OR NOMBRE LIKE %s OR CATEGORIA LIKE %s OR MARCA LIKE %s)"
        params = [f"%{query}%"] * 4
...
def materiales():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cat = request.args.get('cat', '')
        sql = "SELECT * FROM materiales WHERE (REF_MAT LIKE %s OR NOMBRE LIKE %s)"
...
def materiales_nombres():
    ...
    try:
        cursor = conn.cursor()
        sql = "SELECT REF_MAT, NOMBRE FROM materiales WHERE NOMBRE LIKE %s OR REF_MAT LIKE %s LIMIT 10"
...
def base_articles():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cat = request.args.get('cat', '')
        sql = "SELECT * FROM productos WHERE ES_VARIANTE = 'BASE' AND (SKU_REF LIKE %s OR NOMBRE LIKE %s)"
        params = [f"%{query}%", f"%{query}%"]
...
def api_capacidad():
    ...
    conn = get_db_connection()
    if not conn: return jsonify({"capacidad": 0})
    try:
        cursor = conn.cursor(dictionary=True)
...
def api_ventas():
    ...
    try:
        ...
        sql = """
            ...
            AND (p.SKU_REF LIKE %s OR p.NOMBRE LIKE %s)
        """
        params = [f"%{query}%", f"%{query}%"]
...
def api_fabricable():
    ...
    sku = request.args.get('sku', '')
    conn = get_db_connection()
    if not conn: return jsonify({'fabricable': 0, 'error': 'DB Error'}), 500
    try:
        cursor = conn.cursor()
...
def api_get_influencers():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM influencers WHERE nombre LIKE %s OR usuario_ig LIKE %s OR nicho LIKE %s", (f'%{query}%', f'%{query}%', f'%{query}%'))
        rows = cursor.fetchall()
        return jsonify(rows)
    finally:
        conn.close()
...
def api_save_influencer():
    ...
    try:
        ...
        if data.get('id'):
            cursor.execute('''
                UPDATE influencers SET nombre=%s, usuario_ig=%s, email=%s, telefono=%s, vibe_estilo=%s, nicho=%s, seguidores=%s, likes_promedio=%s
                WHERE id=%s
...
def api_ventas_detalle():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM plataformas_ventas WHERE SKU_BASE = %s", (sku,))
        row = cursor.fetchone()
...
def vincular_material():
    ...
    try:
        ...
        cursor.execute("SELECT 1 FROM materiales WHERE REF_MAT = %s", (ref_mat,))
        if not cursor.fetchone():
            return jsonify({"success": False, "error": "Material no encontrado"}), 404
            
        cursor.execute("REPLACE INTO despiece_articulos (SKU_BASE, REF_MAT, CANTIDAD) VALUES (%s, %s, %s)", 
                       (sku_base, ref_mat, float(cantidad)))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
...
def futuros():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cat = request.args.get('cat', '')
        sql = "SELECT id, FECHA, NOMBRE, ESTADO, FOTO_REFERENCIA, CATEGORIA, MARCA, DESCRIPCION, UNIDADES_REALIZADAS FROM futuros_proyectos WHERE (NOMBRE LIKE %s OR CATEGORIA LIKE %s OR MARCA LIKE %s OR ESTADO LIKE %s)"
        params = [f"%{query}%"] * 4
    ...
    try:
        ...
        if data['type'] == 'prod':
            if is_new:
                ...
                cursor.execute("""
                    UPDATE productos SET NOMBRE=%s, PRECIO=%s, CATEGORIA=%s, SUBCATEGORIA=%s, MARCA=%s, COLOR=%s, DIMENSIONES=%s, FESTIVIDAD=%s, DESCRIPCION=%s, STOCK=%s, STOCK_FISICO=%s, FOTO_PORTADA=%s, UNIDADES_REALIZADAS=%s
                    WHERE SKU_REF=%s
            ...
            if is_new:
                ...
                cursor.execute("""
                    UPDATE futuros_proyectos SET NOMBRE=%s, CATEGORIA=%s, SUBCATEGORIA=%s, MARCA=%s, DESCRIPCION=%s, FOTO_REFERENCIA=%s, ESTADO=%s, UNIDADES_REALIZADAS=%s
                    WHERE id=%s
            ...
            if is_new:
                ...
                cursor.execute("""
                    UPDATE materiales SET NOMBRE=%s, UNIDAD=%s, STOCK_ACTUAL=%s, PUNTO_PEDIDO=%s, FOTO=%s, CATEGORIA=%s, SUBCATEGORIA=%s, MARCA=%s, COLOR=%s, DIMENSIONES=%s, FESTIVIDAD=%s
                    WHERE REF_MAT=%s
...
def delete_item():
    ...
    try:
        ...
        cursor.execute(f"SELECT {foto_col} FROM {table} WHERE {pk_col} = %s", (pk_val,))
        row = cursor.fetchone()
        if row and row[0]:
            foto_path = row[0]
    ...
    try:
        ...
        if data.get('id'):
            ...
            cursor.execute('''
                ...
                SET estado_colab=%s, notas=%s, precio_venta=%s, porcentaje_comision=%s
                WHERE id=%s
            ...
            estado_colab = data.get('estado_colab', 'Propuesta enviada')
            estado_pedido = estado_colab_map.get(estado_colab, 'Por empezar')
            cursor.execute('UPDATE pedidos SET estado=%s WHERE colab_id=%s',
            ...
            cursor.execute('SELECT NOMBRE FROM productos WHERE SKU_REF=%s', (data.get('sku_articulo', ''),))
            prod = cursor.fetchone()
            nombre_art = prod[0] if prod else data.get('sku_articulo', '')

            # Obtener nombre del influencer
            cursor.execute('SELECT nombre FROM influencers WHERE id=%s', (data['influencer_id'],))
            inf = cursor.fetchone()
            ...
            # 2. Asegurar columnas necesarias en pedidos (MySQL compatible checking)
            cursor.execute("DESCRIBE pedidos")
            cols = [c[0] for c in cursor.fetchall()]
...
def api_inf_colaboraciones_eliminar():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute('DELETE FROM colaboraciones_influencers WHERE id=%s', (colab_id,))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def api_cliente_encargos_guardar():
    ...
    try:
        ...
        if data.get('id'):
            ...
            cursor.execute('''
                UPDATE encargos_clientes SET estado=%s, notas=%s, cantidad=%s WHERE id=%s
            ...
            cursor.execute('UPDATE pedidos SET estado=%s, notas=%s WHERE encargo_id=%s',
            ...
            # 2. Verificar que la tabla pedidos tiene columna encargo_id y sku_articulo (MySQL compatible)
            cursor.execute("DESCRIBE pedidos")
            cols = [c[0] for c in cursor.fetchall()]
...
def api_cliente_encargos_eliminar():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute('DELETE FROM encargos_clientes WHERE id=%s', (enc_id,))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def api_tareas_guardar():
    ...
    try:
        ...
        if data.get('id'):
            cursor.execute("""
                UPDATE tareas SET descripcion=%s, prioridad=%s, fecha_limite=%s, completada=%s
                WHERE id=%s
...
def api_tareas_eliminar():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM tareas WHERE id=%s", (t_id,))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def api_save_compra():
    ...
    try:
        ...
        if data.get('id'):
            cursor.execute("""
                UPDATE lista_compras SET articulo=%s, cantidad=%s, comprado=%s
                WHERE id=%s
...
def api_delete_compra():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM lista_compras WHERE id=%s", (c_id,))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
