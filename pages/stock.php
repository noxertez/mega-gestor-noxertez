...
<div class="panel-management">
    ...
    <div id="tab-5-4" class="tab-container <?php echo ($current_tab == '5.4') ? 'active' : ''; ?>">
        ...
        <!-- Contenedor del Mapa -->
        <div id="mapaContenedor" style="display:none; margin-bottom: 30px; min-height: 300px;">
            <!-- Estanterías generadas por JS -->
        </div>

        <!-- Tabla de Inventario en Estanterías (Diseño Corregido) -->
        <div class="inventory-shelf-section" style="margin-top: 50px; clear: both; border-top: 2px solid rgba(212,175,55,0.2); padding-top: 30px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h3 style="color: var(--accent-gold); margin: 0; display: flex; align-items: center; gap: 12px; font-family: 'Cinzel', serif;">
                    <i class="fas fa-th-list"></i> CONTENIDO DETALLADO DE ESTANTERÍAS
                </h3>
                <span style="font-size: 0.75rem; opacity: 0.5; background: rgba(255,255,255,0.05); padding: 4px 12px; border-radius: 20px;">
                    Mostrando solo A, B y C
                </span>
            </div>
            
            <div class="scroll-x-wow" style="background: rgba(15, 23, 42, 0.4); border: 1px solid var(--border-glass); border-radius: 16px; overflow-x: auto;">
                <table class="table-wow" style="width: 100%; min-width: 800px; border-collapse: collapse;">
                    <thead>
                        <tr style="background: rgba(0,0,0,0.3);">
                            <th style="padding: 15px; text-align: center; width: 100px; color: var(--accent-gold);">POSICIÓN</th>
                            <th style="padding: 15px; text-align: left;">ARTÍCULO / MATERIAL</th>
                            <th style="padding: 15px; text-align: left;">REFERENCIA</th>
                            <th style="padding: 15px; text-align: center; width: 120px;">TIPO</th>
                            <th style="padding: 15px; text-align: center; width: 80px;">ACCIONES</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                        try {
                            $items_ubicados = $db->query("
                                (SELECT NOMBRE as nombre, SKU_REF as ref, UBICACION_MAP as ubi, 'Producto' as tipo 
                                 FROM productos 
                                 WHERE UBICACION_MAP IS NOT NULL AND UBICACION_MAP != '' 
                                 AND (UBICACION_MAP LIKE 'A%' OR UBICACION_MAP LIKE 'B%' OR UBICACION_MAP LIKE 'C%'))
                                UNION
                                (SELECT NOMBRE as nombre, REF_MAT as ref, ubicacion as ubi, 'Material' as tipo 
                                 FROM materiales 
                                 WHERE ubicacion IS NOT NULL AND ubicacion != '' 
                                 AND (ubicacion LIKE 'A%' OR ubicacion LIKE 'B%' OR ubicacion LIKE 'C%'))
                                ORDER BY ubi ASC, nombre ASC
                            ")->fetchAll();

                            if (empty($items_ubicados)): ?>
                                <tr>
                                    <td colspan="5" style="padding: 40px; text-align: center; opacity: 0.4;">
                                        <i class="fas fa-info-circle"></i> No hay artículos detectados en las estanterías A, B o C.
                                    </td>
                                </tr>
                            <?php else: 
                                foreach($items_ubicados as $i): ?>
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.03);">
                                    <td style="padding: 12px; text-align: center;">
                                        <span style="background: var(--accent-gold); color: #000; padding: 3px 10px; border-radius: 6px; font-weight: 900; font-size: 0.8rem;">
                                            <?php echo htmlspecialchars($i['ubi']); ?>
                                        </span>
                                    </td>
                                    <td style="padding: 12px; font-weight: 500;"><?php echo htmlspecialchars($i['nombre']); ?></td>
                                    <td style="padding: 12px; opacity: 0.6; font-family: 'Courier New', monospace; font-size: 0.8rem;">
                                        <?php echo htmlspecialchars($i['ref']); ?>
                                    </td>
                                    <td style="padding: 12px; text-align: center;">
                                        <span style="font-size: 0.65rem; padding: 4px 8px; border-radius: 6px; font-weight: 800; border: 1px solid <?php echo ($i['tipo']=='Producto' ? '#3b82f6' : '#ef4444'); ?>; color: <?php echo ($i['tipo']=='Producto' ? '#60a5fa' : '#f87171'); ?>; background: rgba(0,0,0,0.2);">
                                            <?php echo strtoupper($i['tipo']); ?>
                                        </span>
                                    </td>
                                    <td style="padding: 12px; text-align: center;">
                                        <button onclick="quitarItemDeEstanteria('<?php echo $i['ref']; ?>', '<?php echo $i['tipo']; ?>', '<?php echo htmlspecialchars($i['nombre']); ?>')" 
                                                class="btn-premium-wow" 
                                                style="padding: 6px 10px; background: rgba(212, 175, 55, 0.1); border: 1px solid rgba(212, 175, 55, 0.3); color: var(--accent-gold);" 
                                                title="Quitar de esta ubicación (El artículo NO se borrará)">
                            ...
                            echo '<tr><td colspan="5" style="padding:20px; color:#ef4444;">Error: '.$e->getMessage().'</td></tr>';
                        } ?>
                    </tbody>
                </table>
            </div>
        ...
        <script>
        async function quitarItemDeEstanteria(ref, tipo, nombre) {
            if (!confirm(`¿Quieres QUITAR el ${tipo}: "${nombre}" de esta estantería? \n\n(El artículo seguirá existiendo en tu inventario, solo se quedará sin ubicación asignada).`)) return;

            try {
                const res = await fetch(`api/index.php?ruta=stock&accion=quitar_ubicacion_inventario&ref=${encodeURIComponent(ref)}&tipo=${tipo}`, {
                    method: 'DELETE'
                });
...
async function panelAsignarMaterial(etiqueta, posId, estNombre) {
  ...
  const d = await r.json();
  if (d.ok) {
    cargarMapa(); // Refrescar iconos en el mapa
...
async function asignarMaterialCelda() {
  ...
  const d = await r.json();
  if (d.ok) { 
    alert(`✅ Material asignado a ${etiqueta}`);
    cargarMapa();
  }
