...
<style>
    ...
    .btn-bulk:hover { transform: scale(1.05); box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
</style>
    ...
    <div class="tab-content-wow">
        ...
        <div id="tab-bloc" class="tab-container <?php echo ($current_tab == 'bloc' || !isset($_GET['tab'])) ? 'active' : ''; ?>">
            <div class="notepad-container">
                <h2 style="margin-top:0; color:#ff6b6b; font-family:sans-serif; text-transform:uppercase; font-size:1rem; border-bottom:2px solid #ff6b6b; padding-bottom:5px; display:flex; justify-content:space-between; align-items:center;">
                    Notas de Voz Rápidas
                    <button onclick="toggleSelectAll()" class="btn-bulk" id="btn-select-all">SELECCIONAR TODO</button>
                </h2>

                <div id="bulk-bar" class="bulk-actions-bar">
                    <span id="bulk-count">0 seleccionadas</span>
                    <button onclick="deleteSelectedNotes()" class="btn-bulk" style="background: #991b1b; color: white;">BORRAR SELECCIONADAS</button>
                </div>
                ...
                <div id="notas-pendientes">
                    ...
                    <?php else: ?>
                        <?php foreach($tareas_pendientes as $t): ?>
                            <div class="note-item" id="note-<?php echo $t['id']; ?>">
                                <div class="note-checkbox-container">
                                    <input type="checkbox" class="note-checkbox" value="<?php echo $t['id']; ?>" onchange="updateBulkBar()">
                                </div>
                                <div style="flex-grow:1; display:flex; justify-content:space-between;" onclick="openNoteModal(<?php echo $t['id']; ?>, '<?php echo addslashes($t['descripcion']); ?>', false)">
                ...
                <?php if (!empty($historial_notas)): ?>
                    ...
                    <div id="notas-historial">
                        <?php foreach($historial_notas as $t): ?>
                            <div class="note-item completed" id="note-<?php echo $t['id']; ?>">
                                <div class="note-checkbox-container">
                                    <input type="checkbox" class="note-checkbox" value="<?php echo $t['id']; ?>" onchange="updateBulkBar()">
                                </div>
                                <div style="flex-grow:1; display:flex; justify-content:space-between;" onclick="openNoteModal(<?php echo $t['id']; ?>, '<?php echo addslashes($t['descripcion']); ?>', true)">
                                    <span class="note-text"><?php echo htmlspecialchars($t['descripcion']); ?></span>
                                    <span class="note-date"><?php echo date('d M', strtotime($t['fecha_creacion'])); ?></span>
                                </div>
                            </div>
...
function updateBulkBar() {
    ...
    if (checked.length > 0) {
        ...
        count.innerText = `${checked.length} seleccionadas`;
    } else {
...
function toggleSelectAll() {
    ...
    btn.innerText = !allChecked ? 'DESELECCIONAR TODO' : 'SELECCIONAR TODO';
    updateBulkBar();
}
...
async function deleteSelectedNotes() {
    ...
    if (confirm(`¿Borrar permanentemente estas ${ids.length} notas?`)) {
        document.getElementById('response').innerText = "Borrando notas...";
        try {
            const res = await fetch(`<?php echo $base_path; ?>api/index.php?ruta=tareas&accion=borrar_masivo`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
            ...
            alert('Error al borrar las notas');
        }
    }
}
