...
echo translate('meta_description', 'Resend the activation email for your World of Warcraft server account.'); ?>">
    <title><?php
echo $site_title_name ." ". translate('page_title', 'Resend Activation Email'); ?></title>
    <style>
        :root{
            --bg-resend-act:url('<?php
echo $base_path; ?>img/backgrounds/bg-register.jpg');
        }
    </style>
</head>
    ...
    <div class="wrapper">
        <div class="form-container">
            <div class="form-section">
                <h2><?php
echo translate('resend_title', 'Resend Activation Email'); ?></h2>
                <?php
if (!empty($errors)): ?>
                    <div class="error">
                        <?php
...
if ($success): ?>
                    <div class="success">
                        <p><?php
echo htmlspecialchars($success); ?></p>
                    </div>
                <?php
endif; ?>
                <form method="POST">
                    <input type="text" name="username" placeholder="<?php
echo translate('username_placeholder', 'Username'); ?>" required value="<?php
echo htmlspecialchars($test_username); ?>">
                    <input type="email" name="email" placeholder="<?php
echo translate('email_placeholder', 'Email'); ?>" required value="<?php
...
echo translate('resend_button', 'Resend Activation Email'); ?></button>
                    <div class="login-link">
                        <?php
echo translate('login_link', 'Already activated?'); ?> <?php
echo sprintf(translate('login_link_text', '<a href="%s">Log in here</a>'), htmlspecialchars($base_path . 'login')); ?>
                    </div>
                </form>
            </div>
        </div>
