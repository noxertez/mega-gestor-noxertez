...
echo translate('meta_description', 'View your World of Warcraft character equipment, stats, and PvP details.'); ?>">
    <meta name="robots" content="index">
    <title><?php
echo $site_title_name ." ". translate('page_title', 'Character Equipment'); ?></title>
    
    <style>
    :root{
            --bg-character:url('<?php
echo $base_path; ?>img/backgrounds/bg-character.jpg');
            --hover-wow-gif: url('<?php
echo $base_path; ?>img/hover_wow.gif');
        }
</style>
    <script>
...
if (isset($items[$slot])): ?>
                        ...
                        <?php
                            ...
                            <div class="empty-slot"><?php
echo translate('slot_empty', 'Empty'); ?></div>
                        <?php
endif; ?>
                    </div>
                </div>
...
if ($error): ?>
            ...
            <?php
                ...
                <div class="character-details">
                    <span class="character-level"><?php
echo translate('level_label', 'Level'); ?> <?= $character['level'] ?? '??' ?> <?= isset($classes[$character['class']]) ? htmlspecialchars($classes[$character['class']]['name']) : translate('class_unknown', 'Unknown') ?></span>
                ...
                <div class="character-image">
                    <img src="<?php
echo $base_path; ?>3dmodels/3d_default.gif" alt="<?php
echo translate('default_character_image', 'Default Character Image'); ?>" class="default-image">
...
if (!$error): ?>
        <div class="tab-nav">
            <button data-tab="stats-tab" class="active"><?php
echo translate('tab_stats', 'Stats'); ?></button>
            <button data-tab="talents-tab" class=""><?php
echo translate('tab_talents', 'Talents'); ?></button>
            <button data-tab="pvp-tab" class=""><?php
echo translate('tab_pvp', 'PVP'); ?></button>
        </div>
...
if ($stats): ?>
                <div class="stats-container">
                    <div class="stats-category">
                        <h3><?php
echo translate('stats_base', 'Base Stats'); ?></h3>
                        <div class="stats-item"><span><?php
echo translate('stat_health', 'Health'); ?></span><span><?= number_format($stats['maxhealth']) ?></span></div>
                        <?php
// Always display Mana if available
                        if ($stats['maxpower1'] > 0):
                        ?>
                            <div class="stats-item"><span><?php
echo translate('stat_mana', 'Mana'); ?></span><span><?= number_format($stats['maxpower1']) ?></span></div>
                        <?php
else: ?>
                            <div class="stats-item"><span><?php
echo translate('stat_mana', 'Mana'); ?></span><span><?php
echo translate('stat_not_available', 'Not Available'); ?></span></div>
                        <?php
endif; ?>
                        <?php
// Map class IDs to their primary power type indices (using PowerType IDs)
                        ...
                        <div class="stats-item"><span><?php
echo translate('stat_strength', 'Strength'); ?></span><span><?= number_format($stats['strength']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_agility', 'Agility'); ?></span><span><?= number_format($stats['agility']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_stamina', 'Stamina'); ?></span><span><?= number_format($stats['stamina']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_intellect', 'Intellect'); ?></span><span><?= number_format($stats['intellect']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_spirit', 'Spirit'); ?></span><span><?= number_format($stats['spirit']) ?></span></div>
                    </div>
                    <div class="stats-category">
                        <h3><?php
echo translate('stats_defense', 'Defense'); ?></h3>
                        <div class="stats-item"><span><?php
echo translate('stat_armor', 'Armor'); ?></span><span><?= number_format($stats['armor']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_block', 'Block'); ?></span><span><?= number_format($stats['blockPct'], 2) ?>%</span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_dodge', 'Dodge'); ?></span><span><?= number_format($stats['dodgePct'], 2) ?>%</span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_parry', 'Parry'); ?></span><span><?= number_format($stats['parryPct'], 2) ?>%</span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_resilience', 'Resilience'); ?></span><span><?= number_format($stats['resilience']) ?></span></div>
                    </div>
                    <div class="stats-category">
                        <h3><?php
echo translate('stats_melee', 'Melee'); ?></h3>
                        <div class="stats-item"><span><?php
echo translate('stat_attack_power', 'Attack Power'); ?></span><span><?= number_format($stats['attackPower']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_crit_chance', 'Crit Chance'); ?></span><span><?= number_format($stats['critPct'], 2) ?>%</span></div>
                    </div>
                    <div class="stats-category">
                        <h3><?php
echo translate('stats_ranged', 'Ranged'); ?></h3>
                        <div class="stats-item"><span><?php
echo translate('stat_ranged_attack_power', 'Attack Power'); ?></span><span><?= number_format($stats['rangedAttackPower']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_ranged_crit_chance', 'Crit Chance'); ?></span><span><?= number_format($stats['rangedCritPct'], 2) ?>%</span></div>
                    </div>
                    <div class="stats-category">
                        <h3><?php
echo translate('stats_resistances', 'Resistances'); ?></h3>
                        <div class="stats-item"><span><?php
echo translate('stat_holy_resistance', 'Holy Resistance'); ?></span><span><?= number_format($stats['resHoly']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_fire_resistance', 'Fire Resistance'); ?></span><span><?= number_format($stats['resFire']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_nature_resistance', 'Nature Resistance'); ?></span><span><?= number_format($stats['resNature']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_frost_resistance', 'Frost Resistance'); ?></span><span><?= number_format($stats['resFrost']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_shadow_resistance', 'Shadow Resistance'); ?></span><span><?= number_format($stats['resShadow']) ?></span></div>
                        <div class="stats-item"><span><?php
echo translate('stat_arcane_resistance', 'Arcane Resistance'); ?></span><span><?= number_format($stats['resArcane']) ?></span></div>
                    </div>
                </div>
            <?php
else: ?>
                <div class="pvp-team"><?php
echo translate('stats_none', 'No Stats Available'); ?></div>
            <?php
endif; ?>
        </div>
        <div id="talents-tab" class="tab-content">
            <div class="pvp-team"><?php
echo translate('talents_coming_soon', 'Talents (Coming Soon)'); ?></div>
        </div>
...
foreach ($pvp_teams as $team): ?>
                    <div class="pvp-team-item">
                        <div class="pvp-team">
                            <?= htmlspecialchars($team['name']) ?> (<?= $team['type'] == 2 ? '2v2' : ($team['type'] == 3 ? '3v3' : '5v5') ?>, <?php
echo translate('pvp_rating', 'Rating'); ?>: <?= $team['rating'] ?>)
                        </div>
...
endforeach; ?>
            <?php
                ...
                <div class="pvp-team"><?php
echo translate('pvp_none', 'No PvP Teams'); ?></div>
            <?php
endif; ?>
            <div class="pvp-kills"><?php
echo translate('pvp_total_kills', 'Total PvP Kills'); ?>: <span><?= number_format($total_kills) ?></span></div>
        </div>
    <?php
endif; ?>
