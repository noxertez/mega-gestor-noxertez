...
echo translate('admin_news_meta_description', 'News Management for Sahtout WoW Server'); ?>">
    <meta name="robots" content="noindex">
    <title><?php
echo translate('admin_news_page_title', 'News Management'); ?></title>
    <link rel="icon" href="<?php
echo $base_path . $site_logo; ?>" type="image/x-icon">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="<?php
echo $base_path; ?>assets/css/admin/anews.css">
    <link rel="stylesheet" href="<?php
...
echo translate('admin_news_title', 'News Management'); ?></h1>
                    <?php
echo $update_message; ?>
                    <!-- Add News Form -->
                    <div class="card">
                        <div class="card-header"><?php
echo translate('admin_news_add_header', 'Add New News'); ?></div>
                        <div class="card-body">
...
echo translate('admin_news_label_title', 'Title'); ?></label>
                                    <input type="text" name="title" class="form-control" required maxlength="100" placeholder="<?php
echo translate('admin_news_placeholder_title', 'Enter news title'); ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label"><?php
echo translate('admin_news_label_slug', 'Slug'); ?></label>
                                    <input type="text" name="slug" class="form-control" maxlength="120" placeholder="<?php
echo translate('admin_news_placeholder_slug', 'Enter slug (optional)'); ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label"><?php
echo translate('admin_news_label_content', 'Content'); ?></label>
                                    <textarea name="content" class="form-control" rows="5" required placeholder="<?php
echo translate('admin_news_placeholder_content', 'Enter news content'); ?>"></textarea>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label"><?php
echo translate('admin_news_label_category', 'Category'); ?></label>
                                    <select name="category" id="category" class="form-select">
                                        <option value="update"><?php
echo translate('admin_news_category_update', 'Update'); ?></option>
                                        <option value="event"><?php
echo translate('admin_news_category_event', 'Event'); ?></option>
                                        <option value="maintenance"><?php
echo translate('admin_news_category_maintenance', 'Maintenance'); ?></option>
                                        <option value="other"><?php
echo translate('admin_news_category_other', 'Other'); ?></option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label"><?php
echo translate('admin_news_label_image', 'Image Upload (JPG, PNG, GIF, max 2MB, Optional)'); ?></label>
...
echo translate('admin_news_image_help', 'Leave blank to use default image (news.png).'); ?></small>
                                    <img id="image_preview" class="image-preview" src="" alt="<?php
echo translate('admin_news_image_preview_alt', 'Image Preview'); ?>">
                                </div>
                                <div class="mb-3">
                                    <div class="form-check">
                                        <input type="checkbox" name="is_important" class="form-check-input" id="is_important">
                                        <label class="form-check-label" for="is_important" style="color: #dc3545;"><?php
echo translate('admin_news_label_is_important', 'Mark as Important'); ?></label>
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-primary"><?php
echo translate('admin_news_add_button', 'Add News'); ?></button>
                            </form>
                        </div>
                    </div>
                    <!-- News List -->
                    <div class="card">
                        <div class="card-header"><?php
echo translate('admin_news_list_header', 'News Articles'); ?></div>
                        <div class="card-body">
                            <div class="table-wrapper">
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th><?php
echo translate('admin_news_table_title', 'Title'); ?></th>
                                            <th><?php
echo translate('admin_news_table_category', 'Category'); ?></th>
                                            <th><?php
echo translate('admin_news_table_posted_by', 'Posted By'); ?></th>
                                            <th><?php
echo translate('admin_news_table_date', 'Date'); ?></th>
                                            <th><?php
echo translate('admin_news_table_important', 'Important'); ?></th>
                                            <th><?php
echo translate('admin_news_table_image', 'Image'); ?></th>
                                            <th><?php
echo translate('admin_news_table_actions', 'Actions'); ?></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
if ($news_result->num_rows === 0): ?>
                                            <tr>
                                                <td colspan="7"><?php
echo translate('admin_news_no_news', 'No news available.'); ?></td>
                                            </tr>
                                        <?php
else: ?>
                                            <?php
...
echo $news['id']; ?>">
                                                    <td><a href="<?php
echo $base_path; ?>news?slug=<?php
echo urlencode(htmlspecialchars($news['slug'])); ?>" class="news-title-link"><?php
echo htmlspecialchars($news['title']); ?></a></td>
                                                    <td><?php
echo translate('admin_news_category_' . $news['category'], ucfirst($news['category'])); ?></td>
                                                    <td><?php
echo htmlspecialchars($news['posted_by']); ?></td>
                                                    <td><?php
echo date('M j, Y H:i', strtotime($news['post_date'])); ?></td>
                                                    <td><span class="<?php
echo $news['is_important'] ? 'status-important' : ''; ?>">
                                                        <?php
echo $news['is_important'] ? translate('admin_news_yes', 'Yes') : translate('admin_news_no', 'No'); ?>
                                                    </span></td>
                                                    <td>
                                                        <img src="<?php
echo htmlspecialchars($news['image_url'] ?? $default_image_url); ?>" alt="<?php
echo translate('admin_news_image_alt', 'News Image'); ?>" style="max-width: 50px; max-height: 50px;">
                                                    </td>
                                                    <td>
                                                        <button class="btn btn-edit" data-bs-toggle="modal" data-bs-target="#editModal-<?php
echo $news['id']; ?>"><?php
echo translate('admin_news_edit_button', 'Edit'); ?></button>
                                                        <button class="btn btn-delete" data-bs-toggle="modal" data-bs-target="#deleteModal-<?php
echo $news['id']; ?>"><?php
echo translate('admin_news_delete_button', 'Delete'); ?></button>
                                                    </td>
                                                </tr>
                                                <!-- Edit Modal -->
                                                <div class="modal fade" id="editModal-<?php
echo $news['id']; ?>" tabindex="-1" aria-labelledby="editModalLabel-<?php
echo $news['id']; ?>" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="editModalLabel-<?php
echo $news['id']; ?>"><?php
echo translate('admin_news_edit_modal_title', 'Edit News: ') . htmlspecialchars($news['title']); ?></h5>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="<?php
echo translate('admin_news_close_button', 'Close'); ?>"></button>
                                                            </div>
...
echo $news['id']; ?>">
                                                                    <input type="hidden" name="csrf_token" value="<?php
echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                                                    <input type="hidden" name="existing_image" id="existing_image_<?php
echo $news['id']; ?>" value="<?php
echo htmlspecialchars($news['image_url'] ?? $default_image_url); ?>">
                                                                    <div class="mb-3">
                                                                        <label class="form-label"><?php
echo translate('admin_news_label_title', 'Title'); ?></label>
                                                                        <input type="text" name="title" class="form-control" value="<?php
echo htmlspecialchars($news['title']); ?>" required maxlength="100" placeholder="<?php
echo translate('admin_news_placeholder_title', 'Enter news title'); ?>">
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <label class="form-label"><?php
...
echo htmlspecialchars($news['slug'] ?? ''); ?>" maxlength="120" placeholder="<?php
echo translate('admin_news_placeholder_slug', 'Enter slug (optional)'); ?>">
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <label class="form-label"><?php
echo translate('admin_news_label_content', 'Content'); ?></label>
                                                                        <textarea name="content" class="form-control" rows="5" required placeholder="<?php
echo translate('admin_news_placeholder_content', 'Enter news content'); ?>"><?php
echo htmlspecialchars($news['content']); ?></textarea>
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <label class="form-label"><?php
echo translate('admin_news_label_category', 'Category'); ?></label>
                                                                        <select name="category" class="form-select">
                                                                            <option value="update" <?php
echo $news['category'] === 'update' ? 'selected' : ''; ?>><?php
echo translate('admin_news_category_update', 'Update'); ?></option>
                                                                            <option value="event" <?php
echo $news['category'] === 'event' ? 'selected' : ''; ?>><?php
echo translate('admin_news_category_event', 'Event'); ?></option>
                                                                            <option value="maintenance" <?php
echo $news['category'] === 'maintenance' ? 'selected' : ''; ?>><?php
echo translate('admin_news_category_maintenance', 'Maintenance'); ?></option>
                                                                            <option value="other" <?php
echo $news['category'] === 'other' ? 'selected' : ''; ?>><?php
echo translate('admin_news_category_other', 'Other'); ?></option>
                                                                        </select>
                                                                    </div>
                                                                    <div class="mb-3">
...
echo translate('admin_news_label_image', 'Image Upload (JPG, PNG, GIF, max 2MB, Optional)'); ?></label>
                                                                        ...
                                                                        <small class="form-text text-muted"><?php
echo translate('admin_news_image_edit_help', 'Leave blank to keep existing image (default: news.png).'); ?></small>
                                                                        <img class="image-preview active" id="image_preview_<?php
echo $news['id']; ?>" src="<?php
echo htmlspecialchars($news['image_url'] ?? $default_image_url); ?>" alt="<?php
echo translate('admin_news_image_preview_alt', 'Image Preview'); ?>">
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <div class="form-check">
                                                                            <input type="checkbox" name="is_important" class="form-check-input" id="is_important_<?php
echo $news['id']; ?>" <?php
echo $news['is_important'] ? 'checked' : ''; ?>>
                                                                            <label class="form-check-label" for="is_important_<?php
echo $news['id']; ?>"><?php
echo translate('admin_news_label_is_important', 'Mark as Important'); ?></label>
                                                                        </div>
                                                                    </div>
...
echo translate('admin_news_cancel_button', 'Cancel'); ?></button>
                                                                        <button type="submit" class="btn btn-primary"><?php
echo translate('admin_news_save_button', 'Save'); ?></button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
...
echo $news['id']; ?>" tabindex="-1" aria-labelledby="deleteModalLabel-<?php
echo $news['id']; ?>" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
...
echo $news['id']; ?>"><?php
echo translate('admin_news_delete_modal_title', 'Delete News: ') . htmlspecialchars($news['title']); ?></h5>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="<?php
echo translate('admin_news_close_button', 'Close'); ?>"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                <p><?php
echo translate('admin_news_delete_confirm', 'Are you sure you want to delete this news article?'); ?></p>
                                                                <form method="POST">
...
if ($total_pages > 1): ?>
                                <nav aria-label="<?php
echo translate('admin_news_pagination_aria', 'News pagination'); ?>">
                                    <ul class="pagination">
                                        <li class="page-item <?php
echo $page <= 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="<?php
echo $base_path; ?>admin/anews?page=<?php
echo $page - 1; ?>" aria-label="<?php
echo translate('admin_news_previous', 'Previous'); ?>">
...
echo $base_path; ?>admin/anews?page=<?php
echo $page + 1; ?>" aria-label="<?php
echo translate('admin_news_next', 'Next'); ?>">
                ...
                addImageInput.addEventListener('change', function() {
                    if (this.files && this.files[0]) {
                        ...
                        if (!allowedTypes.includes(file.type)) {
                            alert('<?php
echo translate('admin_news_js_invalid_file_type', 'Invalid file type. Only JPG, PNG, or GIF allowed.'); ?>');
                            this.value = '';
                            addImagePreview.classList.remove('active');
                            addImagePreview.src = '<?php
echo htmlspecialchars($default_image_url); ?>';
                            return;
                        }
                        if (file.size > maxSize) {
                            alert('<?php
echo translate('admin_news_js_file_size_exceeded', 'File size exceeds 2MB limit.'); ?>');
                            this.value = '';
                            addImagePreview.classList.remove('active');
                            addImagePreview.src = '<?php
echo htmlspecialchars($default_image_url); ?>';
