...
echo translate('meta_description', 'Reset your password for our World of Warcraft server.'); ?>">
    <title><?php
echo $site_title_name ." ". translate('page_title', 'Reset Password'); ?></title>
    <style>
        :root{
            --bg-reset-pw:url('<?php
echo $base_path; ?>img/backgrounds/bg-reset-password.jpg');
        }
    </style>
</head>
    ...
    <div class="wrapper">
        <div class="form-container">
            <div class="form-section">
                <h2><?php
echo translate('reset_title', 'Reset Password'); ?></h2>
                <?php
if (!empty($errors)): ?>
                    <div class="error">
                        <?php
...
echo htmlspecialchars($success); ?></p>
                        <p class="login-link"><?php
echo sprintf(translate('login_link', '<a href="%s">Back to Login</a>'), htmlspecialchars($base_path . 'login')); ?></p>
                    </div>
                    <script>
                        setTimeout(() => {
                            window.location.href = '<?php
echo $base_path; ?>login';
                        }, 3000);
                    </script>
                <?php
else: ?>
...
echo htmlspecialchars($nonce); ?>">
                            <input type="password" name="password" placeholder="<?php
echo translate('password_placeholder', 'New Password'); ?>" required>
                            <input type="password" name="confirm_password" placeholder="<?php
echo translate('confirm_password_placeholder', 'Confirm Password'); ?>" required>
                            <?php
if (defined('RECAPTCHA_ENABLED') && RECAPTCHA_ENABLED): ?>
...
echo translate('reset_button', 'Reset Password'); ?></button>
                            <p class="login-link"><?php
echo sprintf(translate('login_link', '<a href="%s">Back to Login</a>'), htmlspecialchars($base_path . 'login')); ?></p>
                        </form>
                    <?php
