...
echo translate('shop_meta_description', 'Browse and purchase items, mounts, pets, gold, and services for '.$site_title_name . ' WoW Server'); ?>">
    <title><?php
echo $site_title_name ." ".translate('shop_page_title', '- Shop'); ?></title>
...
<body class="shop">
    <div class="shop-container">
        <h1><?php
echo $site_title_name ." ". translate('shop_title', 'Server Shop'); ?></h1>
        <?php
if (!empty($_SESSION['user_id'])): ?>
            <div class="user-balance">
                <span class="points"><i class="fas fa-coins"></i> <?php
echo translate('shop_points', 'Points'); ?>: <?php
echo $points; ?></span>
                <span class="tokens"><i class="fas fa-gem"></i> <?php
echo translate('shop_tokens', 'Tokens'); ?>: <?php
echo $tokens; ?></span>
            </div>
        <?php
else: ?>
            <p class="login-prompt"><?php
echo str_replace('{base_path}', $base_path, translate('shop_login_prompt', 'Please <a href="{base_path}login">log in</a> to purchase items.')); ?></p>
        <?php
endif; ?>
...
foreach ($valid_categories as $category): ?>
                <a href="#" 
                   class="category-button <?php
echo $selected_category === $category ? 'active' : ''; ?>" 
                   data-category="<?php
echo htmlspecialchars($category); ?>">
                    <?php
if (isset($category_images[$category])): ?>
                        <img src="<?php
echo $base_path . $category_images[$category]; ?>" alt="<?php
echo translate('shop_category_' . strtolower($category) . '_icon', htmlspecialchars($category) . ' Icon'); ?>" class="category-icon">
                    <?php
endif; ?>
                    <?php
echo translate('shop_category_' . strtolower($category), htmlspecialchars($category)); ?>
                </a>
            <?php
endforeach; ?>
        </nav>
...
echo translate('shop_no_items', 'No items available.'); ?></p>
        <?php
else: ?>
            <?php
foreach ($items as $category => $category_items): ?>
                <section class="shop-category" data-category="<?php
echo htmlspecialchars($category); ?>" 
                         style="display: <?php
echo ($selected_category === 'All' || $selected_category === $category) ? 'block' : 'none'; ?>">
                    <h2><?php
echo translate('shop_category_' . strtolower($category), htmlspecialchars($category)); ?></h2>
                    <div class="item-grid">
                        <?php
foreach ($category_items as $item): ?>
                            <div class="item-card" data-entry="<?php
echo $item['sit_entry'] ? htmlspecialchars($item['sit_entry']) : ''; ?>">
                                <div class="item-image-container">
                                    <img src="<?php
echo $base_path . ($item['image'] ?? 'img/shop/placeholder.png'); ?>" alt="<?php
echo str_replace('{name}', htmlspecialchars($item['name']), translate('shop_item_image_alt', '{name}')); ?>">
                                    <?php
...
echo htmlspecialchars($item['name']); ?></h3>
                                <p><?php
echo htmlspecialchars($item['description'] ?? translate('shop_no_description', 'No description available.')); ?></p>
                                <?php
if ($category === 'Service'): ?>
                                    <?php
if ($item['level_boost'] !== null): ?>
                                        <p class="level-boost"><?php
echo translate('shop_level_boost', 'Level Boost'); ?>: <?php
echo $item['level_boost']; ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 1): ?>
                                        <p class="rename-character"><?php
echo translate('shop_rename_character', 'Character Rename'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 2): ?>
                                        <p class="reset-spells"><?php
echo translate('shop_reset_spells', 'Reset Spells'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 4): ?>
                                        <p class="reset-talents"><?php
echo translate('shop_reset_talents', 'Reset Talents'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 8): ?>
                                        <p class="customize-character"><?php
echo translate('shop_customize_character', 'Character Customization'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 16): ?>
                                        <p class="reset-pet-talents"><?php
echo translate('shop_reset_pet_talents', 'Reset Pet Talents'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 32): ?>
                                        <p class="first-login"><?php
echo translate('shop_first_login', 'First Login'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 64): ?>
                                        <p class="faction-change"><?php
echo translate('shop_faction_change', 'Faction Change'); ?></p>
                                    <?php
endif; ?>
                                    <?php
if ($item['at_login_flags'] & 128): ?>
                                        <p class="race-change"><?php
echo translate('shop_race_change', 'Race Change'); ?></p>
                                    <?php
endif; ?>
                                <?php
endif; ?>
...
if ($item['point_cost'] > 0): ?>
                                        <span class="points"><i class="fas fa-coins"></i> <?php
echo $item['point_cost']; ?></span>
                                    <?php
endif; ?>
                                    <?php
if ($item['token_cost'] > 0): ?>
                                        <span class="tokens"><i class="fas fa-gem"></i> <?php
echo $item['token_cost']; ?></span>
                                    <?php
endif; ?>
                                </div>
...
if ($item['stock'] !== null): ?>
                                        <span><?php
echo translate('shop_stock', 'Stock'); ?>: <?php
echo $item['stock']; ?></span>
                                    <?php
else: ?>
                                        <span><?php
echo translate('shop_unlimited_stock', 'Unlimited Stock'); ?></span>
                                    <?php
endif; ?>
                                </div>
                                <?php
if (!empty($_SESSION['user_id'])): ?>
                                    <form action="<?php
echo $base_path; ?>buy_item" method="POST" class="purchase-form">
...
if (!empty($characters)): ?>
                                            <select name="character_id" class="character-select" required>
                                                <option value=""><?php
echo translate('shop_select_character', 'Select a Character'); ?></option>
                                                <?php
foreach ($characters as $char): ?>
                                                    <option value="<?php
echo htmlspecialchars($char['id']); ?>">
                                                        <?php
...
endforeach; ?>
                                        ...
                                        <?php
                                            ...
                                            <p class="no-characters"><?php
echo translate('shop_no_characters', 'No characters available.'); ?></p>
                                        <?php
endif; ?>
                                        <button type="submit" class="buy-button" 
                                                <?php
echo ($item['stock'] === 0 && $item['stock'] !== null) || empty($characters) || $cooldown_active ? 'disabled' : ''; ?>
                                                data-item-id="<?php
echo $item['item_id']; ?>">
                                            <?php
echo $cooldown_active ? str_replace('{seconds}', $remaining_cooldown, translate('shop_wait_cooldown', 'Wait {seconds}s')) : translate('shop_buy_now', 'Buy Now'); ?>
                                        </button>
                                    </form>
                                <?php
else: ?>
                                    <a href="<?php
echo $base_path; ?>login" class="login-to-buy-button"><?php
echo translate('shop_login_to_buy', 'Log in to Buy'); ?></a>
                                <?php
endif; ?>
                            </div>
                        <?php
...
endforeach; ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let remainingCooldown = <?php
echo json_encode($remaining_cooldown); ?>;
        let isPurchaseBlocked = <?php
echo json_encode($cooldown_active); ?>;
        ...
        function updateBuyButtons(disabled, seconds) {
            document.querySelectorAll('.buy-button:not([href])').forEach(button => {
                if (!button.hasAttribute('disabled') || button.getAttribute('disabled') === '') {
                    ...
                    button.textContent = disabled ? '<?php
echo translate('shop_wait_cooldown', 'Wait {seconds}s'); ?>'.replace('{seconds}', seconds) : '<?php
echo translate('shop_buy_now', 'Buy Now'); ?>';
        ...
        purchaseForms.forEach(form => {
            form.addEventListener('submit', function(e) {
                ...
                if (characterSelect && !characterSelect.value) {
                    e.preventDefault();
                    showCustomAlert('<?php
echo translate('shop_js_select_character', 'Please select a character to purchase this item.'); ?>', 'info');
                    return;
                }
                if (isPurchaseBlocked) {
                    e.preventDefault();
                    showCustomAlert('<?php
echo translate('shop_js_cooldown_active', 'Please wait {seconds} seconds before making another purchase.'); ?>'.replace('{seconds}', remainingCooldown), 'warning');
                    return;
                }
            });
        });
        ...
        buyButtons.forEach(button => {
            if (<?php
echo json_encode(empty($characters)); ?>) {
                button.addEventListener('click', function(e) {
                    e.preventDefault();
                    showCustomAlert('<?php
echo translate('shop_js_no_characters', 'You have no characters available. Please create a character first.'); ?>', 'warning');
                });
            }
        });
            ...
            alertBox.innerHTML = `
                ...
                <div class="alert-content">
                    ...
                    <div class="alert-actions">
                        <a href="<?php
echo $base_path; ?>login" class="alert-btn login-btn">
    ...
    body {
        background: url('<?php
echo $base_path; ?>img/backgrounds/bg-shop.jpg') no-repeat center center fixed;
