...
<!DOCTYPE html>
<html lang="<?php
echo htmlspecialchars($_SESSION['lang'] ?? 'en', ENT_QUOTES, 'UTF-8'); ?>">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php
echo $site_title_name ." ". translate('vote_title', 'Vote for Epic Rewards'); ?></title>
...
<body>
    <div class="container">
        <div class="page-header">
            <a href="<?php
echo htmlspecialchars($base_path, ENT_QUOTES, 'UTF-8'); ?>"><img src="<?php
echo htmlspecialchars($base_path . $site_logo, ENT_QUOTES, 'UTF-8'); ?>" alt="<?php
echo translate('site_logo_alt', 'Sahtout Server Logo'); ?>" class="logo"></a>
            <h1><?php
echo translate('vote_title_h1', 'Vote for Epic Rewards'); ?></h1>
            <p class="subtitle"><?php
echo translate('vote_subtitle', 'Support our server by voting on top sites and earn exclusive in-game rewards!'); ?></p>
        </div>
        <?php
if (empty($username)): ?>
            <p style="text-align: center; color: var(--light); font-size: 1rem; margin-bottom: 20px;">
                <?php
echo translate('vote_login_prompt', 'Please log in to vote and earn rewards.'); ?>
                <a href="<?php
echo htmlspecialchars($base_path . 'login', ENT_QUOTES, 'UTF-8'); ?>" style="color: var(--accent); text-decoration: underline;">Log in now</a>
            </p>
        <?php
endif; ?>
        <div class="message-box <?php
echo htmlspecialchars($claim_message_type, ENT_QUOTES, 'UTF-8'); ?> <?php
echo $claim_message ? 'show' : ''; ?>">
            <?php
echo $claim_message; ?>
        </div>
        <div class="vote-sites">
            <?php
if (empty($voteSites)): ?>
                <p style="text-align: center; color: var(--light); font-size: 1rem;"><?php
echo translate('vote_no_sites', 'No vote sites available at the moment.'); ?></p>
            <?php
else: ?>
                <?php
foreach ($voteSites as $site): ?>
                    <div class="vote-site">
                        <div class="vote-site-header">
                            <h3 class="vote-site-name"><?php
echo htmlspecialchars($site['site_name'], ENT_QUOTES, 'UTF-8'); ?></h3>
                        </div>
                        <div class="vote-site-image">
                            <img src="<?php
echo htmlspecialchars($site['button_image_url'] ?? $base_path . 'img/default.png', ENT_QUOTES, 'UTF-8'); ?>" alt="<?php
echo translate('vote_site_image_alt', 'Voting Site') . ': ' . htmlspecialchars($site['site_name'], ENT_QUOTES, 'UTF-8'); ?>">
                        </div>
                        <div class="vote-site-content">
                            <div class="vote-site-points-container">
                                <span class="vote-site-points"><?php
echo htmlspecialchars($site['reward_points'], ENT_QUOTES, 'UTF-8'); ?> <?php
echo translate('vote_points_label', 'Vote Points'); ?></span>
                            </div>
...
// Construct the voting URL
                                ...
                                ?>
                                <a href="<?php
echo $vote_url; ?>" class="vote-btn" target="_blank" data-site-name="<?php
echo htmlspecialchars($site['site_name'], ENT_QUOTES, 'UTF-8'); ?>" <?php
echo (empty($username) && $site['uses_callback']) || $site['is_on_cooldown'] ? 'disabled' : ''; ?>><?php
echo translate('vote_button', 'Vote'); ?></a>
                                <?php
if ($account_id > 0): ?>
                                    <button class="claim-btn" onclick="claimRewards(<?php
echo (int)$account_id; ?>, '<?php
echo htmlspecialchars($site['callback_file_name'], ENT_QUOTES, 'UTF-8'); ?>', '<?php
echo htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8'); ?>')" <?php
echo $site['has_unclaimed_rewards'] ? '' : 'disabled'; ?>><?php
echo translate('claim_button', 'Claim'); ?></button>
                                <?php
endif; ?>
                            </div>
                            <p class="cooldown"><?php
echo htmlspecialchars($site['cooldown_hours'], ENT_QUOTES, 'UTF-8'); ?> <?php
echo translate('vote_cooldown_label', 'h cooldown'); ?></p>
                            <?php
if ($account_id > 0): ?>
                                <p class="cooldown-timer" data-site-id="<?php
echo (int)$site['id']; ?>" data-remaining-seconds="<?php
echo (int)$site['remaining_cooldown']; ?>" data-cooldown-hours="<?php
echo (int)$site['cooldown_hours']; ?>">
                                    <?php
echo $site['is_on_cooldown'] ? translate('vote_cooldown_timer', 'Cooldown: Calculating...') : translate('vote_cooldown_ready', 'Ready to vote!'); ?>
                                </p>
                            <?php
endif; ?>
                        </div>
...
echo translate('vote_rewards_title', 'Voting Rewards'); ?></h2>
            <div class="reward-list">
                <div class="reward-item">
                    <div class="reward-icon"><i class="fas fa-coins"></i></div>
                    <h3><?php
echo translate('vote_reward_gold', 'Gold'); ?></h3>
                    <p><?php
echo translate('vote_reward_gold_desc', 'Receive up to 40 gold per vote to boost your in-game wealth.'); ?></p>
                </div>
                <div class="reward-item">
                    <div class="reward-icon"><i class="fas fa-hat-wizard"></i></div>
                    <h3><?php
echo translate('vote_reward_enchants', 'Enchants'); ?></h3>
                    <p><?php
echo translate('vote_reward_enchants_desc', 'Unlock powerful weapon and armor enchants for your characters.'); ?></p>
                </div>
                <div class="reward-item">
                    <div class="reward-icon"><i class="fas fa-dragon"></i></div>
                    <h3><?php
echo translate('vote_reward_mounts', 'Mounts'); ?></h3>
                    <p><?php
echo translate('vote_reward_mounts_desc', 'Gain access to exclusive mounts only available through voting.'); ?></p>
                </div>
                <div class="reward-item">
                    <div class="reward-icon"><i class="fas fa-gem"></i></div>
                    <h3><?php
echo translate('vote_reward_vip_points', 'VIP Points'); ?></h3>
                    <p><?php
echo translate('vote_reward_vip_points_desc', 'Earn points to redeem for special items and perks.'); ?></p>
                </div>
            </div>
        </div>
    </div>
    <div class="modal-overlay">
        <div class="modal-content">
            ...
            <h2><?php
echo translate('vote_modal_title', 'Thank You for Voting!'); ?></h2>
            <p><?php
echo translate('vote_modal_message', 'You\'re being redirected to <span class="site-name"></span> to complete your vote.'); ?></p>
            <button onclick="closeModal()"><?php
echo translate('vote_modal_button', 'Continue'); ?></button>
        </div>
    </div>
    <?php
            ...
            voteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    if (this.disabled) {
                        e.preventDefault();
                        if (messageBox) {
                            messageBox.textContent = '<?php
echo translate('vote_unauthorized', 'Cannot vote during cooldown.'); ?>';
                ...
                if (remainingSeconds > 0) {
                    const interval = setInterval(() => {
                        if (remainingSeconds <= 0) {
                            ...
                            timer.textContent = '<?php
echo translate('vote_cooldown_ready', 'Ready to vote!'); ?>';
                            ...
                            if (response.status === 200) {
                                return {
                                    data: {
                                        status: 'success',
                                        message: '<?php
echo translate('vote_claim_success', 'Rewards claimed successfully!'); ?>'
                                    },
                                    response
                                };
                            }
                ...
                .then(({ data, response }) => {
                    ...
                    messageBox.textContent = data.message || '<?php
echo translate('vote_claim_success', 'Rewards claimed successfully!'); ?>';
                ...
                .catch(error => {
                    ...
                    messageBox.textContent = '<?php
echo translate('vote_claim_error', 'Error claiming rewards: ') ?>' + error.message;
                    messageBox.className = 'message-box error show';
                    messageBox.style.display = 'block';
                    setTimeout(() => {
                        messageBox.classList.remove('show');
