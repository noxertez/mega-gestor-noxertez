...
echo translate('meta_description', 'Request a password reset link for your World of Warcraft server account.'); ?>">
    <title><?php
echo $site_title_name ." ". translate('page_title', 'Forgot Password'); ?></title>
</head>
    <style>
        :root{
            --bg-forgot-pw:url('<?php
echo $base_path; ?>img/backgrounds/bg-password.jpg');
        }
    </style>
</style>
    ...
    <div class="wrapper">
        <div class="form-container">
            <div class="form-section">
                <h2><?php
echo translate('forgot_title', 'Forgot Password'); ?></h2>
                <?php
if (!empty($errors)): ?>
                    <div class="error">
                        <?php
...
echo htmlspecialchars($success); ?></p>
                ...
                <form method="POST">
                    <input type="text" name="username_or_email" placeholder="<?php
echo translate('username_or_email_placeholder', 'Username or Email'); ?>" required value="<?php
echo htmlspecialchars($username_or_email); ?>">
                    <?php
if (defined('RECAPTCHA_ENABLED') && RECAPTCHA_ENABLED): ?>
...
echo translate('send_button', 'Send Reset Link'); ?></button>
                    <p class="login-link">
                        <?php
echo translate('login_link', 'Remembered your password?'); ?>
                        <?php
