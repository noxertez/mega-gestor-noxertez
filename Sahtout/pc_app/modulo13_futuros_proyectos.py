...
class TabRegistro(tk.Frame):
    ...
    def guardar(self):
        ...
        from modulo3_gestion import RUTA_PROYECTOS, RUTA_WEB_PROYECTOS
        import shutil
        foto_original = self.foto_path_var.get()
        final_foto = foto_original

        # 1. SANITIZAR EL NOMBRE PARA LA WEB Y EL ESCRITORIO
        archivo_original = os.path.basename(foto_original)
        archivo_clean = self.gestor.sanitizar_nombre_archivo(archivo_original)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename_nuevo = f"{timestamp}_{archivo_clean}"

        # 2. COPIA AL ESCRITORIO (Local)
        if os.path.exists(foto_original):
            try:
                dest_escritorio = os.path.join(RUTA_PROYECTOS, filename_nuevo)
                shutil.copy2(foto_original, dest_escritorio)
                final_foto = dest_escritorio
            except Exception as e:
                print(f"Error copiando foto local: {e}")

        # 3. COPIA AL SERVIDOR WEB (XAMPP)
        if os.path.exists(RUTA_WEB_PROYECTOS) and os.path.exists(foto_original):
            try:
                dest_web = os.path.join(RUTA_WEB_PROYECTOS, filename_nuevo)
                shutil.copy2(foto_original, dest_web)
                print(f"Copiado a WEB: {dest_web}")
            except Exception as e:
...
class TabBuscadorFuturos(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # self.buscar()  # Desactivado carga inicial por rendimiento
    ...
    def enviar_produccion(self, item):
        ...
        try:
            ...
            cursor.execute("UPDATE futuros_proyectos SET ESTADO='En Proceso' WHERE id=?", (item['id'],))
            conn.commit()
            conn.close()
...
class TabTablaFuturos(tk.Frame):
    ...
    def setup_ui(self):
        # Botones y Filtros superiores
        top = tk.Frame(self, bg=COLOR_FONDO, pady=10)
        top.pack(fill="x")
        
        tk.Label(top, text="Filtrar Categoría:", bg=COLOR_FONDO, font=("Segoe UI", 10, "bold")).pack(side="left", padx=(20, 5))
        self.cb_cat = ttk.Combobox(top, textvariable=self.cat_var, state="readonly")
        self.cb_cat.pack(side="left", padx=5)
        self.actualizar_categorias()
        
        tk.Button(top, text="🔍 Filtrar / Actualizar", command=self.cargar_datos, bg="#3b82f6", fg="white", padx=15).pack(side="left", padx=20)
        ...
        # self.cargar_datos() # Desactivado carga inicial por rendimiento


    def actualizar_categorias(self):
        cats = ["(Todas)"] + self.gestor.obtener_categorias()
        self.cb_cat['values'] = cats
    ...
    def cargar_datos(self):
        ...
        if cat_sel and cat_sel != "(Todas)":
            filtros["CATEGORIA"] = cat_sel
            
        proyectos = self.gestor.buscar_proyectos(filtros)
        for p in proyectos:
