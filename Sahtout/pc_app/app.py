"""
NOXERTEZ v4.0 - GESTOR INTEGRADO (MySQL + SahtoutCMS)
"""
...
try:
    ...
    sys.path.insert(0, _os.path.join(_os.path.dirname(__file__), 'n8n'))
    import modulo_n8n as n8n_mod
    
    print("Todos los modulos cargados correctamente")
except Exception as e:
    print(f"Error cargando modulos: {e}")
    print("Asegúrate de que todos los archivos modulo*.py están en la misma carpeta")
...
class CatalogoApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("🎨 Noxertez v4.0 — MySQL Edition")
        ...
        self.root.after(500, lambda: Notificacion.mostrar(
            self.root,
            "✅ v4.0: MySQL Conectado | SahtoutCMS Activo",
            'exito'
        ))
        ...
        try:
            import sys, os as _os
            _db = _os.path.join(_os.path.dirname(__file__), 'catalogo.db')
            # inicializar_tablas ya no es necesario con MySQL (tablas creadas en Fase 1)
    ...
    def _crear_contenido_tabla(self, parent):
        ...
        self.lbl_total_tabla = tk.Label(parent, text="Total: 0 artículos", font=('Segoe UI', 10), bg='#0f172a', fg='#94a3b8')
        ...
        tk.Button(btns_frame, text="🔄 Actualizar Tabla", font=('Segoe UI', 10, 'bold'), 
                   command=self.recargar_datos_tabla, bg='#1e293b', fg='white', padx=10).pack(side=tk.LEFT, padx=5)
    ...
    def crear_tab_stock(self):
        """Peestaña de Gestión de Stock (MÓDULO 12)"""
        self.tab_stock = TabStock(self.notebook, self.gestor, self)
        self.notebook.add(self.tab_stock.frame, text="5. 📦 Stock")
    ...
    def crear_tab_n8n(self):
        ...
        self.notebook.add(tab, text="12. 🤖 Automatización")
        ...
        btn_refrescar = tk.Button(est_frame, text='🔄 Refrescar',
                                  font=('Segoe UI', 9), bg='#1e293b', fg='white', padx=8,
                                  relief=tk.FLAT)
        ...
        botones_data = [
            ...
            ('🔄  Reiniciar n8n',            '#f97316', self.reiniciar_n8n,
             'Reinicia el servicio n8n\n(Mata y arranca de nuevo)'),
        ]
        ...
        for (fname, nombre, desc) in flujos:
            fila = tk.Frame(flujos_frame, bg='#1e293b', bd=1, relief=tk.RIDGE)
            fila.pack(fill=tk.X, pady=3)
            tk.Label(fila, text=nombre, font=('Segoe UI', 10, 'bold'),
                     bg='#1e293b', fg='#a5b4fc', anchor='w', width=38).pack(side=tk.LEFT, padx=10, pady=6)
            tk.Label(fila, text=desc, font=('Segoe UI', 9),
                     bg='#1e293b', fg='#94a3b8', anchor='w').pack(side=tk.LEFT, padx=4)
            ...
            tk.Label(fila, text=existe, font=('Segoe UI', 9, 'bold'),
                     bg='#1e293b', fg='#4ade80' if '✅' in existe else '#ef4444').pack(side=tk.RIGHT, padx=12)
        ...
        def cargar_notificaciones():
            try:
                notifs = n8n_mod.obtener_notificaciones_nuevas(_db_path)
        ...
        tareas_lista = tk.Listbox(tareas_outer, font=('Segoe UI', 10),
                                  bg='#1e293b', fg='#f1f5f9', relief=tk.FLAT,
                                  borderwidth=1, selectbackground='#6366f1', selectforeground='white')
        ...
        def cargar_tareas():
            try:
                ...
                conn = mysql.connector.connect(host='localhost', database='noxertez',
                    user='noxertez_user', password='Noxertez2024!', charset='utf8mb4')
        ...
        tk.Button(btn_box, text='🔄 Actualizar Todo',
                  font=('Segoe UI', 9), bg='#1e293b', fg='white', relief=tk.FLAT, padx=8,
        ...
        def marcar_hecho():
            ...
            try:
                ...
                conn = mysql.connector.connect(host='localhost', database='noxertez',
                    user='noxertez_user', password='Noxertez2024!', charset='utf8mb4')
                cur = conn.cursor()
                cur.execute("UPDATE tareas SET completada = 1 WHERE descripcion = %s", (texto,))
                conn.commit()
                conn.close()
                cargar_tareas()
        ...
        def eliminar_tarea():
            ...
            texto = tareas_lista.get(sel[0]).replace('• ', '')
            try:
                # SQLite eliminado — ahora usa MySQL via modulo3_gestion
                conn = __import__("mysql.connector", fromlist=["connect"]).connect(**{"host":"localhost","database":"noxertez","user":"noxertez_user","password":"Noxertez2024!","charset":"utf8mb4"})
                conn.cursor(dictionary=True).execute("DELETE FROM tareas WHERE descripcion = ?", (texto,))
                conn.commit()
                conn.close()
                cargar_tareas()
            except Exception: pass
        ...
        tk.Button(btn_box, text='✅ Hecho',
                  font=('Segoe UI', 9, 'bold'), bg='#166534', fg='white', relief=tk.FLAT, padx=12,
                  command=marcar_hecho).pack(side=tk.LEFT, padx=2)
        
        tk.Button(btn_box, text='🗑️ Eliminar',
                  font=('Segoe UI', 9), bg='#991b1b', fg='white', relief=tk.FLAT, padx=12,
    ...
    def reiniciar_n8n(self):
        """Reinicia el proceso n8n local."""
        if not messagebox.askyesno("Confirmar", "¿Deseas reiniciar el servicio n8n?"):
            return
        
        Notificacion.mostrar(self.root, "⏳ Reiniciando n8n...", 'info')
        self.root.update()
        
        try:
            # Importar aquí por si acaso
            import sys, os as _os
            sys.path.insert(0, _os.path.join(_os.path.dirname(__file__), 'n8n'))
            import modulo_n8n as n8n_mod
            
            # Detener e iniciar
            self.n8n_process = n8n_mod.reiniciar_n8n_local()
            
            if self.n8n_process:
                self.root.after(3000, lambda: Notificacion.mostrar(self.root, "✅ n8n Reiniciado", 'exito'))
            else:
                Notificacion.mostrar(self.root, "❌ Error al reiniciar n8n", 'error')
        except Exception as e:
            messagebox.showerror("Error", f"Fallo al reiniciar n8n: {e}")
    ...
    def crear_formulario(self, parent):
        ...
        log_frame = tk.Frame(form, bg='#1e293b', bd=1, relief=tk.RIDGE)
        log_frame.grid(row=5, column=0, columnspan=4, sticky="ew", pady=10, padx=5)
        
        tk.Label(log_frame, text="📦 LOGÍSTICA (PARA ENVÍOS):", font=('Segoe UI', 10, 'bold'), bg='#1e293b', fg='#94a3b8').pack(side=tk.LEFT, padx=5)
        
        tk.Label(log_frame, text="Peso (Kg):", bg='#1e293b', fg='#f1f5f9').pack(side=tk.LEFT, padx=(10,0))
        self.ent_peso_prod = ttk.Entry(log_frame, width=6)
        self.ent_peso_prod.pack(side=tk.LEFT, padx=5)
        self.ent_peso_prod.insert(0, "0.5")

        tk.Label(log_frame, text="Medidas Paquete (LxAnxAl):", bg='#1e293b', fg='#f1f5f9').pack(side=tk.LEFT, padx=(10,0))
        ...
        tk.Label(log_frame, text="x", bg='#1e293b', fg='#f1f5f9').pack(side=tk.LEFT)
    ...
    def crear_panel_variantes(self, parent):
        ...
        # Lista de variantes
        self.container_variantes = tk.Frame(self.frame_variantes, bg='#0f172a')
        self.container_variantes.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        self.lbl_no_vars = tk.Label(self.container_variantes, text="Selecciona un producto base para ver sus variantes", fg='#94a3b8', bg='#0f172a')
