...
class TabStock:
    ...
    def crear_tab_inventario(self):
        ...
        self.ent_mat_marca.bind("<FocusOut>", self.on_cat_sub_change)
        ...
        tk.Button(form, text="💾 GUARDAR MATERIAL", bg="#10b981", fg="white", 
                  command=self.guardar_material, font=("Segoe UI", 10, "bold")).grid(row=6, column=4, padx=5, pady=10)
        
        tk.Button(form, text="✨ NUEVO", bg="#64748b", fg="white", 
                  command=self.limpiar_formulario_material, font=("Segoe UI", 10, "bold")).grid(row=6, column=3, padx=5, pady=10, sticky="e")
    ...
    def on_cat_sub_change(self, event=None):
        ...
        if (cat or sub or marca) and not self.ent_mat_id.get():
            num = self.gestor_mat.obtener_siguiente_numero_material()
            
            c_sigla = cat.replace(' ', '')[:3].upper() if cat else "GEN"
            s_sigla = sub.replace(' ', '')[:3].upper() if sub else "GEN"
            m_sigla = marca.replace(' ', '')[:3].upper() if marca else "GEN"
            
            sku = f"M-{m_sigla}{c_sigla}{s_sigla}{str(num).zfill(4)}"
    ...
    def limpiar_formulario_material(self):
        ...
        self.ent_mat_cat.delete(0, tk.END)
        self.ent_mat_sub.delete(0, tk.END)
