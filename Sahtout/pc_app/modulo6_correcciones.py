    ...
    def cargar_productos(self, productos):
        ...
        for i, prod in enumerate(productos):
            try:
                ...
                # Tag alternado
                tag = 'evenrow' if insertados % 2 == 0 else 'oddrow'
                
                self.tree.insert('', tk.END, text=str(insertados+1), values=valores, tags=(tag,))
                insertados += 1
            except Exception as e:
                print(f"⚠️ Error cargando fila {i} (SKU: {prod.get('SKU_REF')}): {e}")
