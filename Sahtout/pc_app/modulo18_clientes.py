...
class GestorClientes:
    ...
    def asignar_articulo(self, id_cliente, sku):
        ...
        cursor = conn.cursor(dictionary=True)
        try:
            cursor.execute("INSERT IGNORE INTO cliente_articulos (id_cliente, sku_articulo) VALUES (?, ?)", (id_cliente, sku))
            conn.commit()
    ...
    def crear_pedido_manual(self, id_cliente, sku, tipo_trabajo):
        ...
        try:
            ...
            if tipo_trabajo == "Stock (Listo)":
                cursor.execute("UPDATE productos SET STOCK_FISICO = MAX(0, IFNULL(STOCK_FISICO, 0) - 1) WHERE SKU_REF = ?", (sku,))
                estado = "Listo para entrega" # Nombre correcto para el tablero Kanban
...
class TabClientes(tk.Frame):
    ...
    def abrir_editor_pedido(self, id_pedido):
        ...
        def save():
            try:
                ...
                conn.execute("""
                    UPDATE pedidos 
                    SET sku_articulo = ?, total = ?, estado = ?, notas = ?
                    WHERE id = ?
    ...
    def on_cliente_select(self, event):
        ...
        conn = get_db_connection()
        c = conn.execute("SELECT * FROM clientes WHERE id = ?", (id_c,)).fetchone()
        conn.close()
    ...
    def buscar_articulos_avanzado(self):
        ...
        try:
            ...
            params = []
            if sku: query += " AND SKU_REF LIKE ?"; params.append(f'%{sku}%')
            if nom: query += " AND NOMBRE LIKE ?"; params.append(f'%{nom}%')
            if cat: query += " AND CATEGORIA LIKE ?"; params.append(f'%{cat}%')
            if sub: query += " AND SUBCATEGORIA LIKE ?"; params.append(f'%{sub}%')
            
            res = conn.execute(query + " LIMIT 50", params).fetchall()
            conn.close()
            for r in res:
    ...
    def cargar_historial_pedidos(self, id_cliente):
        ...
        try:
            ...
            pedidos_activos = conn.execute("""
                ...
                FROM pedidos 
                WHERE id_cliente = ? AND estado != 'Entregado'
                ORDER BY id DESC
            """, (id_cliente,)).fetchall()
            
            for p in pedidos_activos:
                self.tree_enc.insert("", tk.END, values=(p['id'], p['fecha_inicio'], p['estado'], p['sku_articulo'] or '--', f"{p['total'] or 0}€"))

            # Pedidos Finalizados (Comprados)
            pedidos_fin = conn.execute("""
            ...
            for p in pedidos_fin:
                ...
                self.tree_com.insert("", tk.END, values=(p['id'], p['fecha_inicio'], p['sku_articulo'] or '--', f"{p['costo_envio'] or 0}€", detalles_envio))
