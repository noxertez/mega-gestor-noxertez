...
class GestorVentas:
        ...
        try:
            ...
            if cursor.fetchone()[0] == 0:
                plats_iniciales = [
                    ...
                    ('trendioff_piedras', 'Trendioff Piedras'),
                ...
                for i, (col, vis) in enumerate(plats_iniciales):
                    cursor.execute("INSERT INTO plataformas_config (nombre_columna, nombre_visible, orden) VALUES (?, ?, ?)", (col, vis, i))
            ...
            # SKU_BASE es el vínculo con la tabla productos
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='plataformas_ventas'")
            ...
            if not table_exists:
                ...
                for plat in plats:
                    query += f", {plat}_ESTADO TEXT, {plat}_PRECIO REAL"
                query += ")"
                cursor.execute(query)
            else:
                # Asegurar que todas las columnas configuradas existen
                cursor.execute("PRAGMA table_info(plataformas_ventas)")
                cols_db = [c[1] for c in cursor.fetchall()]
                for plat in plats:
                    est_col = f"{plat}_ESTADO"
                    pre_col = f"{plat}_PRECIO"
                    url_col = f"{plat}_URL"
                    if est_col not in cols_db:
                        cursor.execute(f"ALTER TABLE plataformas_ventas ADD COLUMN {est_col} TEXT")
                    if pre_col not in cols_db:
                        cursor.execute(f"ALTER TABLE plataformas_ventas ADD COLUMN {pre_col} REAL")
                    if url_col not in cols_db:
                        cursor.execute(f"ALTER TABLE plataformas_ventas ADD COLUMN {url_col} TEXT")
    ...
    def obtener_plataformas(self):
        ...
        try:
            ...
            return [dict(nombre_columna=r[0], nombre_visible=r[1]) for r in cursor.fetchall()]
        except:
            return []
        finally:
            conn.close()
    ...
    def agregar_plataforma(self, nombre_visible):
        ...
        try:
            ...
            cursor.execute("SELECT 1 FROM plataformas_config WHERE nombre_columna = ?", (nombre_col,))
            if cursor.fetchone():
            ...
            cursor.execute("INSERT INTO plataformas_config (nombre_columna, nombre_visible, orden) VALUES (?, ?, ?)", 
                         (nombre_col, nombre_visible, max_orden + 1))
            
            # Agregar columnas a tabla ventas
            cursor.execute(f"ALTER TABLE plataformas_ventas ADD COLUMN {nombre_col}_ESTADO TEXT")
            cursor.execute(f"ALTER TABLE plataformas_ventas ADD COLUMN {nombre_col}_PRECIO REAL")
    ...
    def renombrar_plataforma(self, id_plat, nuevo_nombre_visible):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT nombre_columna FROM plataformas_config WHERE id = ?", (id_plat,))
            old_res = cursor.fetchone()
            if not old_res: return False, "No se encontró la plataforma"
            old_col = old_res[0]

            if old_col != nuevo_nombre_col:
                # Renombrar columnas en sqlite
                cursor.execute(f"ALTER TABLE plataformas_ventas RENAME COLUMN {old_col}_ESTADO TO {nuevo_nombre_col}_ESTADO")
                cursor.execute(f"ALTER TABLE plataformas_ventas RENAME COLUMN {old_col}_PRECIO TO {nuevo_nombre_col}_PRECIO")

            cursor.execute("UPDATE plataformas_config SET nombre_columna = ?, nombre_visible = ? WHERE id = ?", 
    ...
    def obtener_datos_ventas(self, sku_base=None):
        ...
        try:
            ...
            cursor = conn.cursor()
            if sku_base:
                cursor.execute("SELECT * FROM plataformas_ventas WHERE SKU_BASE = ?", (sku_base,))
                row = cursor.fetchone()
    ...
    def guardar_datos_ventas(self, datos):
        ...
        # Sanitización agresiva para MySQL: evitar 'Incorrect integer value: \'\'' 
        # Convertir cualquier cadena vacía en columnas que parecen numéricas a 0
        for key in list(datos.keys()):
            if datos[key] == '':
                # Si es un campo de precio, stock, unidades o contiene 'vntd'
                k = key.upper()
                if 'PRECIO' in k or 'STOCK' in k or 'UNIDADES' in k or 'VNTD' in k or 'CANTIDAD' in k:
                    datos[key] = 0
        ...
        try:
            ...
            if prod_updates:
                # Determinar el tipo de placeholder según la conexión
                placeholder = "%s" if hasattr(conn, 'cursor') and not isinstance(conn, sqlite3.Connection) else "?"
                set_clause = ", ".join([f"{col} = {placeholder}" for col in prod_updates.keys()])
                params = list(prod_updates.values()) + [sku_base]
                cursor.execute(f"UPDATE productos SET {set_clause} WHERE SKU_REF = {placeholder}", params)

            # Verificar si existe en plataformas_ventas
            cursor.execute("SELECT 1 FROM plataformas_ventas WHERE SKU_BASE = ?", (sku_base,))
            ...
            if len(datos) > 1: # Queda al menos SKU_BASE y otro
                ...
                if exists:
                    set_clause_v = ", ".join([f"{col} = {placeholder}" for col in cols if col != 'SKU_BASE'])
                    params_v = [datos[col] for col in cols if col != 'SKU_BASE'] + [sku_base]
                    cursor.execute(f"UPDATE plataformas_ventas SET {set_clause_v} WHERE SKU_BASE = {placeholder}", params_v)
                else:
                    placeholders_v = ", ".join([placeholder] * len(cols))
                    cursor.execute(f"INSERT INTO plataformas_ventas ({', '.join(cols)}) VALUES ({placeholders_v})", list(datos.values()))
    ...
    def importar_desde_excel(self, excel_path):
        ...
        try:
            ...
            mapeo = {
                ...
                'trendioff piedras': ('trendioff_piedras_ESTADO', 'precio.12')
            }
...
class TabVentas:
    ...
    def cargar_detalle_articulo(self, prod_full):
        ...
        for plat, (var_est, var_pre) in self.plats_widgets.items():
    ...
    def _crear_detalle(self, parent):
        ...
        for i, plat_info in enumerate(self.gestor_ventas.columnas_plataformas):
            ...
            self.plats_widgets[plat] = (var_estado, var_precio)
