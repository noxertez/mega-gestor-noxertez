...
class InfluencerManager:
        ...
        try:
            if data.get('id'):
                cursor.execute('''
                    ...
                    nombre=?, usuario_ig=?, email=?, telefono=?, vibe_estilo=?, nicho=?, seguidores=?, likes_promedio=?
                    WHERE id=?
            ...
            return False, "El usuario de Instagram ya existe"
        finally:
            conn.close()
    ...
    def eliminar_influencer(self, id_influencer):
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('DELETE FROM influencers WHERE id=?', (id_influencer,))
        cursor.execute('DELETE FROM colaboraciones_influencers WHERE influencer_id=?', (id_influencer,))
        conn.commit()
        conn.close()
    ...
    def obtener_colaboraciones(self, id_influencer):
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM colaboraciones_influencers WHERE influencer_id=? ORDER BY fecha_envio DESC', (id_influencer,))
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]
    ...
    def cargar_colaboraciones(self):
        ...
        for c in colabs:
            ...
            self.vista_colabs.insert('', tk.END, values=(
                ...
                f"{c['costo_produccion']:.2f}€", f"{beneficio:.2f}€",
