...
if ($metodo === 'GET') {
    ...
    if ($accion === 'mapa_estanterias') {
        ...
        // Obtener TODAS las etiquetas que tienen algun articulo o material asignado (con o sin stock)
        $etiquetasConArt = $db->query("
            SELECT DISTINCT UBICACION_MAP as etiqueta
            FROM productos 
            WHERE UBICACION_MAP IS NOT NULL AND UBICACION_MAP != ''
            UNION
            SELECT DISTINCT ubicacion as etiqueta
            FROM materiales
...
elseif ($metodo === 'DELETE') {
    ...
    // ── DELETE quitar_ubicacion_inventario (CORREGIDO) ───────────────────
    if ($accion === 'quitar_ubicacion_inventario') {
        $ref  = $_GET['ref'] ?? '';
        $tipo = $_GET['tipo'] ?? ''; // 'Producto' o 'Material'

        if (!$ref) { echo json_encode(['ok'=>false, 'error'=>'Referencia requerida']); exit(); }

        if ($tipo === 'Producto') {
            // Solo quitamos la ubicación, NO borramos el producto
            $db->prepare("UPDATE productos SET UBICACION_MAP = NULL WHERE SKU_REF = ?")->execute([$ref]);
        } else {
            // Solo quitamos la ubicación del material
            $db->prepare("UPDATE materiales SET ubicacion = NULL WHERE REF_MAT = ?")->execute([$ref]);
        }
        echo json_encode(['ok' => true]); exit();
    }
}
